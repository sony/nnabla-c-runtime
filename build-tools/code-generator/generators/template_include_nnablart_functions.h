...
typedef struct rt_param_t {{
  ...
  void (*set)(struct rt_param_t*, nn_size_t, float); ///< Get data from position
}} rt_param_t;

#define GET_FLOAT(xParam, xPos) (*((float*)((xParam)->data) + (xPos)))
float getFloat(rt_param_t* param, nn_size_t pos);
#define GET_INT16(xParam, xPos) ((xParam)->coefficient * (float)(*((int16_t*)((xParam)->data) + (xPos))))
float getInt16(rt_param_t* param, nn_size_t pos);
#define GET_INT8(xParam, xPos) ((xParam)->coefficient * (float)(*((int8_t*)((xParam)->data) + (xPos))))
