            ...
            if 'argument' in func:
                ...
                for an, arg in func['argument'].items():
                    ...
                    if arg['Type'] == 'bool':
                        ...
                        defines.append('  rt_list_t {}; ///< Original type is [{}]'.format(an, arg['Type']))
                ...
                defines.append('void init{0}Config({0}Config* config, {1});'.format(fn, ','.join(argstrings)))
                defines.append('void init{}LocalContext(Function* f);'.format(fn))
                defines.append('void free{}LocalContext(Function* f);'.format(fn))

            defines.append('void exec{}(Function* f);'.format(fn))
            defines.append('')
        defines.append('')
