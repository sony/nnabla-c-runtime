...
void allocate_function_context(nn_network_t *n, nn_function_t *function,
  ...
  case NN_FUNCTION_AFFINE: { // Affine
    function_context->exec_func = exec_affine;
    function_context->free_local_context_func = free_affine_local_context;
  ...
  case NN_FUNCTION_CONVOLUTION: { // Convolution
    function_context->exec_func = exec_convolution;
    function_context->free_local_context_func = free_convolution_local_context;
  ...
  case NN_FUNCTION_DEPTHWISE_CONVOLUTION: { // DepthwiseConvolution
    function_context->exec_func = exec_depthwise_convolution;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_DECONVOLUTION: { // Deconvolution
    function_context->exec_func = exec_deconvolution;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_DEPTHWISE_DECONVOLUTION: { // DepthwiseDeconvolution
    function_context->exec_func = exec_depthwise_deconvolution;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_MAX_POOLING: { // MaxPooling
    function_context->exec_func = exec_max_pooling;
    function_context->free_local_context_func = free_max_pooling_local_context;
  ...
  case NN_FUNCTION_AVERAGE_POOLING: { // AveragePooling
    function_context->exec_func = exec_average_pooling;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_GLOBAL_AVERAGE_POOLING: { // GlobalAveragePooling
    function_context->exec_func = exec_global_average_pooling;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_SUM_POOLING: { // SumPooling
    function_context->exec_func = exec_sum_pooling;
    function_context->free_local_context_func = free_sum_pooling_local_context;
  ...
  case NN_FUNCTION_UNPOOLING: { // Unpooling
    function_context->exec_func = exec_unpooling;
    function_context->free_local_context_func = free_unpooling_local_context;
  ...
  case NN_FUNCTION_EMBED: { // Embed
    function_context->exec_func = exec_embed;
    function_context->free_local_context_func = free_embed_local_context;
    function_context->func.local_context = 0;
    allocate_embed_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_SIGMOID: { // Sigmoid
    function_context->exec_func = exec_sigmoid;
    function_context->free_local_context_func = free_sigmoid_local_context;
    function_context->func.local_context = 0;
    allocate_sigmoid_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_SWISH: { // Swish
    function_context->exec_func = exec_swish;
    function_context->free_local_context_func = free_swish_local_context;
    function_context->func.local_context = 0;
    allocate_swish_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_TANH: { // Tanh
    function_context->exec_func = exec_tanh;
    function_context->free_local_context_func = free_tanh_local_context;
    function_context->func.local_context = 0;
    allocate_tanh_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_RELU: { // ReLU
    function_context->exec_func = exec_relu;
    function_context->free_local_context_func = free_relu_local_context;
  ...
  case NN_FUNCTION_LEAKY_RELU: { // LeakyReLU
    function_context->exec_func = exec_leaky_relu;
    function_context->free_local_context_func = free_leaky_relu_local_context;
  ...
  case NN_FUNCTION_SOFTMAX: { // Softmax
    function_context->exec_func = exec_softmax;
    function_context->free_local_context_func = free_softmax_local_context;
  ...
  case NN_FUNCTION_ELU: { // ELU
    function_context->exec_func = exec_elu;
    function_context->free_local_context_func = free_elu_local_context;
  ...
  case NN_FUNCTION_SELU: { // SELU
    function_context->exec_func = exec_selu;
    function_context->free_local_context_func = free_selu_local_context;
  ...
  case NN_FUNCTION_CRELU: { // CReLU
    function_context->exec_func = exec_crelu;
    function_context->free_local_context_func = free_crelu_local_context;
  ...
  case NN_FUNCTION_CELU: { // CELU
    function_context->exec_func = exec_celu;
    function_context->free_local_context_func = free_celu_local_context;
  ...
  case NN_FUNCTION_PRELU: { // PReLU
    function_context->exec_func = exec_prelu;
    function_context->free_local_context_func = free_prelu_local_context;
  ...
  case NN_FUNCTION_BATCH_NORMALIZATION: { // BatchNormalization
    function_context->exec_func = exec_batch_normalization;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_MEAN_SUBTRACTION: { // MeanSubtraction
    function_context->exec_func = exec_mean_subtraction;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_SUM: { // Sum
    function_context->exec_func = exec_sum;
    function_context->free_local_context_func = free_sum_local_context;
  ...
  case NN_FUNCTION_MEAN: { // Mean
    function_context->exec_func = exec_mean;
    function_context->free_local_context_func = free_mean_local_context;
  ...
  case NN_FUNCTION_MAX: { // Max
    function_context->exec_func = exec_max;
    function_context->free_local_context_func = free_max_local_context;
  ...
  case NN_FUNCTION_MIN: { // Min
    function_context->exec_func = exec_min;
    function_context->free_local_context_func = free_min_local_context;
  ...
  case NN_FUNCTION_PROD: { // Prod
    function_context->exec_func = exec_prod;
    function_context->free_local_context_func = free_prod_local_context;
  ...
  case NN_FUNCTION_REDUCE_SUM: { // ReduceSum
    function_context->exec_func = exec_reduce_sum;
    function_context->free_local_context_func = free_reduce_sum_local_context;
    function_context->func.local_context = 0;
    allocate_reduce_sum_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_REDUCE_MEAN: { // ReduceMean
    function_context->exec_func = exec_reduce_mean;
    function_context->free_local_context_func = free_reduce_mean_local_context;
    function_context->func.local_context = 0;
    allocate_reduce_mean_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_ADD2: { // Add2
    function_context->exec_func = exec_add2;
    function_context->free_local_context_func = free_add2_local_context;
  ...
  case NN_FUNCTION_BC_ADD2: { // BcAdd2
    function_context->exec_func = exec_bc_add2;
    function_context->free_local_context_func = free_bc_add2_local_context;
    function_context->func.local_context = 0;
    allocate_bc_add2_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_SUB2: { // Sub2
    function_context->exec_func = exec_sub2;
    function_context->free_local_context_func = free_sub2_local_context;
    function_context->func.local_context = 0;
    allocate_sub2_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_MUL2: { // Mul2
    function_context->exec_func = exec_mul2;
    function_context->free_local_context_func = free_mul2_local_context;
    function_context->func.local_context = 0;
    allocate_mul2_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_DIV2: { // Div2
    function_context->exec_func = exec_div2;
    function_context->free_local_context_func = free_div2_local_context;
    function_context->func.local_context = 0;
    allocate_div2_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_POW2: { // Pow2
    function_context->exec_func = exec_pow2;
    function_context->free_local_context_func = free_pow2_local_context;
    function_context->func.local_context = 0;
    allocate_pow2_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_ADD_SCALAR: { // AddScalar
    function_context->exec_func = exec_add_scalar;
    function_context->free_local_context_func = free_add_scalar_local_context;
  ...
  case NN_FUNCTION_MUL_SCALAR: { // MulScalar
    function_context->exec_func = exec_mul_scalar;
    function_context->free_local_context_func = free_mul_scalar_local_context;
  ...
  case NN_FUNCTION_POW_SCALAR: { // PowScalar
    function_context->exec_func = exec_pow_scalar;
    function_context->free_local_context_func = free_pow_scalar_local_context;
  ...
  case NN_FUNCTION_R_SUB_SCALAR: { // RSubScalar
    function_context->exec_func = exec_r_sub_scalar;
    function_context->free_local_context_func = free_r_sub_scalar_local_context;
  ...
  case NN_FUNCTION_R_DIV_SCALAR: { // RDivScalar
    function_context->exec_func = exec_r_div_scalar;
    function_context->free_local_context_func = free_r_div_scalar_local_context;
  ...
  case NN_FUNCTION_R_POW_SCALAR: { // RPowScalar
    function_context->exec_func = exec_r_pow_scalar;
    function_context->free_local_context_func = free_r_pow_scalar_local_context;
  ...
  case NN_FUNCTION_SIGN: { // Sign
    function_context->exec_func = exec_sign;
    function_context->free_local_context_func = free_sign_local_context;
  ...
  case NN_FUNCTION_MINIMUM2: { // Minimum2
    function_context->exec_func = exec_minimum2;
    function_context->free_local_context_func = free_minimum2_local_context;
    function_context->func.local_context = 0;
    allocate_minimum2_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_MAXIMUM2: { // Maximum2
    function_context->exec_func = exec_maximum2;
    function_context->free_local_context_func = free_maximum2_local_context;
    function_context->func.local_context = 0;
    allocate_maximum2_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_MINIMUM_SCALAR: { // MinimumScalar
    function_context->exec_func = exec_minimum_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_MAXIMUM_SCALAR: { // MaximumScalar
    function_context->exec_func = exec_maximum_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_LOGICAL_AND: { // LogicalAnd
    function_context->exec_func = exec_logical_and;
    function_context->free_local_context_func = free_logical_and_local_context;
    function_context->func.local_context = 0;
    allocate_logical_and_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_LOGICAL_OR: { // LogicalOr
    function_context->exec_func = exec_logical_or;
    function_context->free_local_context_func = free_logical_or_local_context;
    function_context->func.local_context = 0;
    allocate_logical_or_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_LOGICAL_XOR: { // LogicalXor
    function_context->exec_func = exec_logical_xor;
    function_context->free_local_context_func = free_logical_xor_local_context;
    function_context->func.local_context = 0;
    allocate_logical_xor_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_EQUAL: { // Equal
    function_context->exec_func = exec_equal;
    function_context->free_local_context_func = free_equal_local_context;
    function_context->func.local_context = 0;
    allocate_equal_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_NOT_EQUAL: { // NotEqual
    function_context->exec_func = exec_not_equal;
    function_context->free_local_context_func = free_not_equal_local_context;
    function_context->func.local_context = 0;
    allocate_not_equal_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_GREATER_EQUAL: { // GreaterEqual
    function_context->exec_func = exec_greater_equal;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_GREATER: { // Greater
    function_context->exec_func = exec_greater;
    function_context->free_local_context_func = free_greater_local_context;
    function_context->func.local_context = 0;
    allocate_greater_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_LESS_EQUAL: { // LessEqual
    function_context->exec_func = exec_less_equal;
    function_context->free_local_context_func = free_less_equal_local_context;
    function_context->func.local_context = 0;
    allocate_less_equal_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_LESS: { // Less
    function_context->exec_func = exec_less;
    function_context->free_local_context_func = free_less_local_context;
    function_context->func.local_context = 0;
    allocate_less_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_LOGICAL_AND_SCALAR: { // LogicalAndScalar
    function_context->exec_func = exec_logical_and_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_LOGICAL_OR_SCALAR: { // LogicalOrScalar
    function_context->exec_func = exec_logical_or_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_LOGICAL_XOR_SCALAR: { // LogicalXorScalar
    function_context->exec_func = exec_logical_xor_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_EQUAL_SCALAR: { // EqualScalar
    function_context->exec_func = exec_equal_scalar;
    function_context->free_local_context_func = free_equal_scalar_local_context;
  ...
  case NN_FUNCTION_NOT_EQUAL_SCALAR: { // NotEqualScalar
    function_context->exec_func = exec_not_equal_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_GREATER_EQUAL_SCALAR: { // GreaterEqualScalar
    function_context->exec_func = exec_greater_equal_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_GREATER_SCALAR: { // GreaterScalar
    function_context->exec_func = exec_greater_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_LESS_EQUAL_SCALAR: { // LessEqualScalar
    function_context->exec_func = exec_less_equal_scalar;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_LESS_SCALAR: { // LessScalar
    function_context->exec_func = exec_less_scalar;
    function_context->free_local_context_func = free_less_scalar_local_context;
  ...
  case NN_FUNCTION_LOGICAL_NOT: { // LogicalNot
    function_context->exec_func = exec_logical_not;
    function_context->free_local_context_func = free_logical_not_local_context;
    function_context->func.local_context = 0;
    allocate_logical_not_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_CONSTANT: { // Constant
    function_context->exec_func = exec_constant;
    function_context->free_local_context_func = free_constant_local_context;
  ...
  case NN_FUNCTION_ABS: { // Abs
    function_context->exec_func = exec_abs;
    function_context->free_local_context_func = free_abs_local_context;
    function_context->func.local_context = 0;
    allocate_abs_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_EXP: { // Exp
    function_context->exec_func = exec_exp;
    function_context->free_local_context_func = free_exp_local_context;
    function_context->func.local_context = 0;
    allocate_exp_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_LOG: { // Log
    function_context->exec_func = exec_log;
    function_context->free_local_context_func = free_log_local_context;
    function_context->func.local_context = 0;
    allocate_log_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_IDENTITY: { // Identity
    function_context->exec_func = exec_identity;
    function_context->free_local_context_func = free_identity_local_context;
    function_context->func.local_context = 0;
    allocate_identity_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_BATCH_MATMUL: { // BatchMatmul
    function_context->exec_func = exec_batch_matmul;
    function_context->free_local_context_func = free_batch_matmul_local_context;
  ...
  case NN_FUNCTION_ROUND: { // Round
    function_context->exec_func = exec_round;
    function_context->free_local_context_func = free_round_local_context;
    function_context->func.local_context = 0;
    allocate_round_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_CONCATENATE: { // Concatenate
    function_context->exec_func = exec_concatenate;
    function_context->free_local_context_func = free_concatenate_local_context;
  ...
  case NN_FUNCTION_SPLIT: { // Split
    function_context->exec_func = exec_split;
    function_context->free_local_context_func = free_split_local_context;
  ...
  case NN_FUNCTION_STACK: { // Stack
    function_context->exec_func = exec_stack;
    function_context->free_local_context_func = free_stack_local_context;
  ...
  case NN_FUNCTION_SLICE: { // Slice
    function_context->exec_func = exec_slice;
    function_context->free_local_context_func = free_slice_local_context;
  ...
  case NN_FUNCTION_TRANSPOSE: { // Transpose
    function_context->exec_func = exec_transpose;
    function_context->free_local_context_func = free_transpose_local_context;
  ...
  case NN_FUNCTION_BROADCAST: { // Broadcast
    function_context->exec_func = exec_broadcast;
    function_context->free_local_context_func = free_broadcast_local_context;
  ...
  case NN_FUNCTION_ONE_HOT: { // OneHot
    function_context->exec_func = exec_one_hot;
    function_context->free_local_context_func = free_one_hot_local_context;
  ...
  case NN_FUNCTION_FLIP: { // Flip
    function_context->exec_func = exec_flip;
    function_context->free_local_context_func = free_flip_local_context;
  ...
  case NN_FUNCTION_SHIFT: { // Shift
    function_context->exec_func = exec_shift;
    function_context->free_local_context_func = free_shift_local_context;
  ...
  case NN_FUNCTION_RESHAPE: { // Reshape
    function_context->exec_func = exec_reshape;
    function_context->free_local_context_func = free_reshape_local_context;
  ...
  case NN_FUNCTION_MATRIX_DIAG: { // MatrixDiag
    function_context->exec_func = exec_matrix_diag;
    function_context->free_local_context_func = free_matrix_diag_local_context;
    function_context->func.local_context = 0;
    allocate_matrix_diag_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_MATRIX_DIAG_PART: { // MatrixDiagPart
    function_context->exec_func = exec_matrix_diag_part;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_DROPOUT: { // Dropout
    function_context->exec_func = exec_dropout;
    function_context->free_local_context_func = free_dropout_local_context;
  ...
  case NN_FUNCTION_TOP_K_DATA: { // TopKData
    function_context->exec_func = exec_top_k_data;
    function_context->free_local_context_func = free_top_k_data_local_context;
  ...
  case NN_FUNCTION_TOP_K_GRAD: { // TopKGrad
    function_context->exec_func = exec_top_k_grad;
    function_context->free_local_context_func = free_top_k_grad_local_context;
  ...
  case NN_FUNCTION_RAND: { // Rand
    function_context->exec_func = exec_rand;
    function_context->free_local_context_func = free_rand_local_context;
  ...
  case NN_FUNCTION_RANDINT: { // Randint
    function_context->exec_func = exec_randint;
    function_context->free_local_context_func = free_randint_local_context;
  ...
  case NN_FUNCTION_RANDN: { // Randn
    function_context->exec_func = exec_randn;
    function_context->free_local_context_func = free_randn_local_context;
  ...
  case NN_FUNCTION_RANDOM_CROP: { // RandomCrop
    function_context->exec_func = exec_random_crop;
    function_context->free_local_context_func = free_random_crop_local_context;
  ...
  case NN_FUNCTION_RANDOM_FLIP: { // RandomFlip
    function_context->exec_func = exec_random_flip;
    function_context->free_local_context_func = free_random_flip_local_context;
  ...
  case NN_FUNCTION_RANDOM_SHIFT: { // RandomShift
    function_context->exec_func = exec_random_shift;
    function_context->free_local_context_func = free_random_shift_local_context;
  ...
  case NN_FUNCTION_IMAGE_AUGMENTATION: { // ImageAugmentation
    function_context->exec_func = exec_image_augmentation;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_SIGMOID_CROSS_ENTROPY: { // SigmoidCrossEntropy
    function_context->exec_func = exec_sigmoid_cross_entropy;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_BINARY_CROSS_ENTROPY: { // BinaryCrossEntropy
    function_context->exec_func = exec_binary_cross_entropy;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_SOFTMAX_CROSS_ENTROPY: { // SoftmaxCrossEntropy
    function_context->exec_func = exec_softmax_cross_entropy;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_CATEGORICAL_CROSS_ENTROPY: { // CategoricalCrossEntropy
    function_context->exec_func = exec_categorical_cross_entropy;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_SQUARED_ERROR: { // SquaredError
    function_context->exec_func = exec_squared_error;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_ABSOLUTE_ERROR: { // AbsoluteError
    function_context->exec_func = exec_absolute_error;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_HUBER_LOSS: { // HuberLoss
    function_context->exec_func = exec_huber_loss;
    function_context->free_local_context_func = free_huber_loss_local_context;
  ...
  case NN_FUNCTION_EPSILON_INSENSITIVE_LOSS: { // EpsilonInsensitiveLoss
    function_context->exec_func = exec_epsilon_insensitive_loss;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_KL_MULTINOMIAL: { // KLMultinomial
    function_context->exec_func = exec_kl_multinomial;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_BINARY_SIGMOID: { // BinarySigmoid
    function_context->exec_func = exec_binary_sigmoid;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_BINARY_TANH: { // BinaryTanh
    function_context->exec_func = exec_binary_tanh;
    function_context->free_local_context_func = free_binary_tanh_local_context;
    function_context->func.local_context = 0;
    allocate_binary_tanh_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_BINARY_CONNECT_AFFINE: { // BinaryConnectAffine
    function_context->exec_func = exec_binary_connect_affine;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_BINARY_CONNECT_CONVOLUTION: { // BinaryConnectConvolution
    function_context->exec_func = exec_binary_connect_convolution;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_BINARY_WEIGHT_AFFINE: { // BinaryWeightAffine
    function_context->exec_func = exec_binary_weight_affine;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_BINARY_WEIGHT_CONVOLUTION: { // BinaryWeightConvolution
    function_context->exec_func = exec_binary_weight_convolution;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_INQ_AFFINE: { // INQAffine
    function_context->exec_func = exec_inq_affine;
    function_context->free_local_context_func = free_inq_affine_local_context;
  ...
  case NN_FUNCTION_INQ_CONVOLUTION: { // INQConvolution
    function_context->exec_func = exec_inq_convolution;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_FIXED_POINT_QUANTIZE: { // FixedPointQuantize
    function_context->exec_func = exec_fixed_point_quantize;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_POW2_QUANTIZE: { // Pow2Quantize
    function_context->exec_func = exec_pow2_quantize;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_TOP_N_ERROR: { // TopNError
    function_context->exec_func = exec_top_n_error;
    function_context->free_local_context_func = free_top_n_error_local_context;
  ...
  case NN_FUNCTION_BINARY_ERROR: { // BinaryError
    function_context->exec_func = exec_binary_error;
    function_context->free_local_context_func = free_binary_error_local_context;
    function_context->func.local_context = 0;
    allocate_binary_error_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_CONFUSION_MATRIX: { // ConfusionMatrix
    function_context->exec_func = exec_confusion_matrix;
    function_context->free_local_context_func =
  ...
  case NN_FUNCTION_VAT_NOISE: { // VATNoise
    function_context->exec_func = exec_vat_noise;
    function_context->free_local_context_func = free_vat_noise_local_context;
  ...
  case NN_FUNCTION_UNLINK: { // Unlink
    function_context->exec_func = exec_unlink;
    function_context->free_local_context_func = free_unlink_local_context;
    function_context->func.local_context = 0;
    allocate_unlink_local_context(&function_context->func);
  } break;

  case NN_FUNCTION_SINK: { // Sink
    function_context->exec_func = exec_sink;
    function_context->free_local_context_func = free_sink_local_context;
