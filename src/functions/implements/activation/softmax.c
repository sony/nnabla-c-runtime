...
rt_function_error_t allocate_softmax_local_context(rt_function_t *f) {
  softmax_local_context_t *context = (softmax_local_context_t *)(f->local_context);
  softmax_private_t *private = malloc(sizeof(softmax_private_t));
  if (private == 0) {
    return RT_FUNCTION_ERROR_MALLOC;
  }
  const int axis = context->axis;
  const int size = calc_shape_size(f->inputs[0]->shape);
  const int size_axis = axis <= 0 ? size : shape_product_of(f->inputs[0], axis, f->inputs[0]->shape.size);
  ...
  private->specified_axis_size = size_axis;
  private->output_size = size / private->batch_size / private->specified_axis_size;
  if (private->batch_size * private->specified_axis_size * private->output_size != size) {
    return RT_FUNCTION_ERROR_INVALID_SHAPE;
  }
  ((softmax_local_context_t *)(f->local_context))->private = (void *)private;
...
rt_function_error_t exec_softmax(rt_function_t *f) {
  softmax_local_context_t *context = (softmax_local_context_t *)(f->local_context);
  softmax_private_t *private =
      (softmax_private_t *)(context->private);
  const float *const x = (float *)(f->inputs[0]->data);
  float *const y = (float *)(f->outputs[0]->data);
  const int batch_size = private->batch_size;
