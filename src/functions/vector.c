...
static void calc_broadcast_position(rt_list_t broadcast_position, rt_list_t broadcast_shape, rt_list_t position)
{
    int bi, pi;
...

{
    ...
    for(i = 0; i < c.shape_length; i++) {
        ...
        for(j = 0; j < c.num_of_inputs; j++) {
            calc_broadcast_position(c.input_positions[j], c.input_shapes[j], c.position);
            c.input_values[j] = inputs[j].get(&(inputs[j]), shape_to_pos(c.input_shapes[j], c.input_positions[j]));
        }
        calc_broadcast_position(c.output_position, c.output_shape, c.position);
        output.set(&output, shape_to_pos(c.output_shape, c.output_position), calc_func(c.num_of_inputs, c.input_values));
    }
}
...
vector_average_context_t init_vector_average_context(rt_list_t in_shape, rt_list_t axes_shape)
{
    ...
    for(i = 0; i < c.rest_axes.size; i++) {
        while(find_num_in_shape(c.calc_axes, w_pos) >= 0 && w_pos < in_shape.size) {
            w_pos += 1;
        }
